<template>
    ...
    <div class="relative h-screen text-white">
      ...
      <div class="relative z-10 p-4 space-y-4 bg-gray-800 bg-opacity-90 rounded-b-2xl">
        ...
        <div class="flex justify-between mt-4 mx-1">
            <button
                class="bg-gray-800 rounded-full px-8 py-2"
                :class="{ 'bg-yellow-400 text-black': filterType === 'all' }"
                @click="setFilter('all')"
            >
                All
            </button>
            <button
                class="bg-gray-800 rounded-full px-8 py-2"
                :class="{ 'bg-yellow-400 text-black': filterType === 'nearby' }"
            ...
            <button
                class="bg-gray-800 rounded-full px-8 py-2"
                :class="{ 'bg-yellow-400 text-black': filterType === 'available' }"
      ...
      <div class="fixed bottom-0 left-0 right-0 z-20 p-4">
        <div class="bg-gray-900 bg-opacity-80 rounded-lg p-4 space-y-2">
          <div v-for="parking in filteredParkingSpots" :key="parking.id" class="border-t border-gray-700 py-2">
            <p class="text-lg">{{ parking.name }}</p>
            <p class="text-sm text-gray-400">{{ parking.distance }} km away</p>
            <p :class="['text-sm', parking.status.toLowerCase() === 'available' ? 'text-green-400' : 'text-blue-400']">
              {{ parking.status }}
            </p>
          </div>
        </div>
        ...
        <div class="flex justify-between bg-gray-800 px-4 py-5 text-white rounded-full mt-4">
          <router-link to="/customer" class="flex-1 text-center" :class="{ 'text-purple-500': $route.path === '/customer' }">Home</router-link>
          <router-link to="/customer/parking" class="flex-1 text-center" :class="{ 'text-purple-500': $route.path === '/customer/parking' }">Parking</router-link>
          <router-link to="/customer/wallet" class="flex-1 text-center" :class="{ 'text-purple-500': $route.path === '/customer/wallet' }">Wallet</router-link>
          <router-link to="/customer/profile" class="flex-1 text-center" :class="{ 'text-purple-500': $route.path === '/customer/profile' }">Profile</router-link>
        </div>
      </div>
    </div>
  </template>
  ...
  const fetchParkingZones = async () => {
    try {
        ...
        if (response.parking_zones && Array.isArray(response.parking_zones)) {
        parkingSpots.value = response.parking_zones;
        } else {
