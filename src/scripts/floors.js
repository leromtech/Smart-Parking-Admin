...
const createFloor = async (parkingZoneId, floorData) => {
    try {
        ...
        fd.append('capacity', floorData.capacity || 0);
        
        // Append vehicle type IDs as array
        if (floorData.vehicle_type_ids && Array.isArray(floorData.vehicle_type_ids)) {
            floorData.vehicle_type_ids.forEach((id) => {
                fd.append('vehicle_type_ids[]', id);
            });
        }
        
        const { data } = await api.post(`parking-zones/${parkingZoneId}/floors`, fd);
        if (data.success) {
            return data;
        }
    } catch (error) {
...
const updateFloor = async (parkingZoneId, floorId, floorData) => {
    try {
        ...
        const { data } = await api.post(`parking-zones/${parkingZoneId}/floors/${floorId}`, fd);
        if (data.success) {
            return data;
        }
    } catch (error) {
...
const getAvailableVehicleTypes = async () => {
    try {
        ...
        try {
            const { data } = await api.get('parking-zones/floors/available-vehicle-types');
            if (data.success) {
            ...
            if (data.vehicle_types) {
                availableVehicleTypes.value = data.vehicle_types.data || data.vehicle_types;
