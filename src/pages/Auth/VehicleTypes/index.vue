<template>
    <div class="flex flex-col w-full h-full">
        <div class="flex md:flex-row flex-col gap-4 mt-4">
            ...
            <div class="flex flex-col md:w-[20%] w-full">
                <label for="filterByAddress">Filter by Space Occupied</label>
                <select class="w-full h-full px-2 md:py-0 py-2" name="filterByType" v-model="filters.type"
                    ...
                    <option v-for="_option in space_occupied" :key="_option">
                        {{ _option }}
                    </option>
                </select>
            </div>
    ...
    <Modal v-model:open="openCreate">
        <create class="bg-white p-8" @created="fetchData" />
    </Modal>
...
const space_occupied = ref([])

const columns = [
    { name: 'name', label: 'TYPE', field: (row) => row.name },
    { name: 'space_occupied', label: 'SPACE OCCUPIED', field: (row) => row.space_occupied },
    { name: 'created_at', label: 'CREATED AT', field: (row) => row.created_at.split('T')[0] + ' ' + row.created_at.split('T')[1].split('.')[0] },
]
...
const fetchData = async () => {
    const { data } = await api.get('vehicle-types', { params: { filters: filters.value, pagination: pagination.value } })
    console.log(data)
    total.value = data.vehicle_types.total
    items_num.value = data.vehicle_types.data.length

    rows.value = data.vehicle_types.data

    space_occupied.value = data.space_occupied_options
}
