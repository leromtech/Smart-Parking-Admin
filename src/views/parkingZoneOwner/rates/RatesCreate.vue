    ...
    <div class="flex flex-col gap-4 md:w-[500px] text-sm text-neutral-500">
      ...
      <div class="flex flex-col items-start justify-center gap-1">
        ...
        <Select
          ...
          optionLabel="name"
          fluid
          placeholder="Select a vehicle type for the policy"
          v-model="ratePolicyForm.vehicle_type">
          <template #empty>
            <div class="text-sm text-neutral-500">No vehicle types found</div>
          </template>
        </Select>
      </div>
...
watch(
  () => ratePolicyForm.value.scheme_type,
  (newVal) => {
    console.log(newVal);
