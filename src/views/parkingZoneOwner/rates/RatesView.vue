  ...
  <div class="flex flex-col gap-2">
      ...
      <template #header>
        <div class="flex flex-row items-center justify-between w-full">
          ...
          <Button
            icon="pi pi-plus"
            rounded
            @click="addingPolicy = true"></Button>
        </div>
      </template>
    </Panel>

    <DataTable
      v-if="parking_zone?.rates?.length > 0"
      :value="parking_zone?.rates"
        ...
        <template #body="slotProps">
          <div class="flex flex-row gap-2">
            ...
            <Button
              ...
              size="small"
              severity="danger"
              @click="() => deleteInterval(slotProps.data.id)"
              variant="outlined" />
          </div>
        </template>
      </Column>
      ...
      <template #expansion="slotProps">
        <div class="p-4 bg-gray-50 rounded">
          <div
            v-if="
              slotProps.data.fixed_rate !== '0.00' ||
              slotProps.data.fixed_rate === 0
            ">
            <div class="flex justify-between items-center mb-4">
              ...
              <Button
                ...
                size="small"
                @click="
                  () => addIntervalLocal(slotProps.data.id, parking_zone.id)
                "
            ...
            <div
              v-if="
                slotProps.data.intervals && slotProps.data.intervals.length > 0
              "
              class="mb-4">
              <DataTable
                :value="slotProps.data.intervals"
                class="mb-4"
                size="small">
                <Column header="Rate Amount">
                  <template #body="intervalProps">
                    <InputGroup class="w-32">
                      ...
                      <InputNumber
                        v-model="
                          slotProps.data.intervals[intervalProps.index]
                            .rate_amount
                        "
                        mode="currency"
                        currency="INR"
                ...
                <Column header="Duration">
                  <template #body="intervalProps">
                    <div class="flex items-center gap-2">
                      ...
                      <Select
                        :options="dailyIntervals"
                        placeholder="Unit"
                        v-model="
                          slotProps.data.intervals[intervalProps.index].interval
                        "
                        class="w-24"
                        size="small" />
                    </div>
                  ...
                  <template #body="intervalProps">
                    <div class="flex gap-1">
                      <Button
                        ...
                        size="small"
                        severity="success"
                        @click="() => updateInterval(intervalProps.data)" />
                      <Button
                        icon="pi pi-trash"
                        size="small"
                        severity="danger"
                        @click="() => deleteInterval(intervalProps.data.id)" />
                    </div>
                  </template>
                </Column>
              </DataTable>
              ...
              <div class="flex gap-4 items-end">
                <div class="flex flex-col gap-2">
                  ...
                  <InputGroup class="w-32">
                    <InputGroupAddon>₹</InputGroupAddon>
                    <InputNumber
                      v-model="intervalForm.rate_amount"
                      mode="currency"
                      currency="INR"
                      :minFractionDigits="2"
                      placeholder="0.00" />
            ...
            <div
              v-if="
                (!slotProps.data.intervals ||
                  slotProps.data.intervals.length === 0) &&
                activeRateId !== slotProps.data.id
              "
          ...
          <div v-else class="text-center py-4 text-gray-500">
            ...
            <p>
              This rate uses a fixed pricing model (₹{{
                slotProps.data.fixed_rate
              }})
            </p>
          </div>
        </div>
      ...
      <template #empty>
        <div class="text-center py-8">
          ...
          <p class="text-gray-400">
            Click the + button to create your first rate
          </p>
        </div>
      </template>
    </DataTable>
    ...
    <Panel v-else>
      <div class="text-center py-8">
        ...
        <Button
          icon="pi pi-plus"
          label="Create First Rate"
          @click="addingPolicy = true" />
      </div>
    </Panel>

    <Dialog
      v-model:visible="addingPolicy"
      modal
      header="Create a new policy"
...
const dailyIntervals = ["second", "minute", "hour"];
