    ...
    <Panel>
      ...
      <DataTable
        ...
        <Column field="capacity" header="Capacity" sortable>
          <template #body="slotProps">
            {{ slotProps.data.total_capacity || 0 }}
          </template>
        </Column>
        <Column field="capacity" header="Reserved for app" sortable>
          <template #body="slotProps">
            {{ slotProps.data.reserved_for_app || 0 }}
          </template>
        </Column>
        <Column header="Vehicle Types">
          <template #body="slotProps">
            <div class="flex flex-wrap gap-1">
              <span
                v-if="
                  slotProps.data.vehicle_types &&
                  slotProps.data.vehicle_types.length > 0
                ">
                <Tag
                  v-for="vt in slotProps.data.vehicle_types"
                  :key="vt.id"
                  :value="vt.name"
                  severity="info"
                  class="mr-1" />
