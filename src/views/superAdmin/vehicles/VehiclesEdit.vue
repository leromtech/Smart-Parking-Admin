...
const props = defineProps(['item', 'vehicleTypes'])
...
onMounted(async () => {
    ...
    form.value.registration_no = props.item.registration_no
    form.value.vehicle_type = props.item.vehicle_type.id
    form.value.user_id = props.item.user.id
})
