  ...
  <div class="flex flex-col gap-2" v-if="form">
    <div v-if="form" class="flex flex-row gap-4 items-center">
      ...
      <font-awesome-icon
        ...
        class="w-5 h-5 hover:text-neutral-700 cursor-pointer"
        @click="() => (editing_name = !editing_name)"
        v-if="!editing_name"
      />
      <div v-else>
        <font-awesome-icon :icon="['fas', 'check']" class="w-5 h-5 hover:text-neutral-700 cursor-pointer" @click="saveParkingZone" />
        <font-awesome-icon :icon="['fas', 'xmark']" class="w-5 h-5 text-red-700 cursor-pointer" @click="() => (editing_name = false)" />
      </div>
    </div>
    ...
    <div class="border flex flex-col p-2">
      <div class="flex flex-row justify-between items-center border-b py-2">
        ...
        <button class="border bg-blue-400 hover:bg-blue-500 text-white rounded-md px-2 py-1" @click="saveParkingZone">SAVE</button>
      </div>
    ...
    <div class="border flex flex-col">
      <div class="flex flex-row justify-between">
        ...
        <button class="border bg-blue-400 hover:bg-blue-500 text-white rounded-md px-2 py-1" @click="createBookingRate">ADD</button>
      </div>
      ...
      <div class="border m-2 bg-stone-200 pl-4" v-else>NO ITEMS</div>
    </div>
    ...
    <div class="border flex flex-col">
      <div class="flex flex-col p-2">
        <div class="flex flex-row justify-between">
          ...
          <button class="border bg-blue-400 hover:bg-blue-500 text-white rounded-md px-2 py-1" @click="createRate">ADD</button>
        </div>
          ...
          <div v-for="(rate, rateIndex) in rates" :key="rate.id" class="p-2 border m-2 bg-white">
            <div class="border-b flex flex-row justify-between items-center pb-2">
              ...
              <div class="flex gap-2">
                <button class="border bg-blue-400 hover:bg-blue-500 text-white rounded-md px-2 py-1" @click="updateInterval">SAVE</button>
                <font-awesome-icon
                  :icon="['far', 'trash-can']"
                  class="text-red-700 w-5 h-5 mt-2 hover:text-red-500"
            ...
            <div v-for="(interval, intervalIndex) in rate.intervals" :key="interval.id || intervalIndex" class="p-1 justify-between flex gap-4">
              <input type="text" v-model="rates[rateIndex].intervals[intervalIndex].duration" class="border rounded-sm p-1 w-full text-center" />
              <select v-model="rates[rateIndex].intervals[intervalIndex].interval" class="border rounded-sm p-1 w-full text-center">
                <option v-for="interval_option in interval_options" :key="interval_option.label" :value="interval_option.label">
                  {{ interval_option.label }}
                </option>
              </select>
              <input type="text" v-model="rates[rateIndex].intervals[intervalIndex].rate_amount" class="border rounded-sm p-1 w-full text-center" />
              <font-awesome-icon
                :icon="['far', 'trash-can']"
                class="text-red-700 w-5 h-5 mt-2 hover:text-red-500"
            ...
            <div class="border p-2 w-full flex items-center justify-center hover:bg-slate-200" @click="() => createNewInterval(rate.id)">
              <font-awesome-icon :icon="['fas', 'plus']" class="text-gray-700 w-5 h-5" />
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="p-2 border">
      <div class="flex flex-row items-center justify-between border-b">
        ...
        <button class="border bg-blue-400 hover:bg-blue-500 text-white rounded-md px-2 py-1" @click="addManager">ADD</button>
      </div>
        ...
        <template #actions="{ row }">
          <td align="right" class="py-4">
            <div class="flex flex-row items-center justify-end pr-2 gap-2">
              <button @click="() => deleteManager(row.laravel_through_key)" class="rounded-full hover:bg-slate-100">
    ...
    <Modal v-model="openCreateRate">
      <create_rate
        :parking-zone-id="parking_zone_id"
        @close="
          async () => {
            await getRates();
            openCreateRate = !openCreateRate;
          }
        "
      ></create_rate>
    </Modal>
      ...
      <create_manager
        ...
        @close="
          async () => {
            ...
            open_create_manager = !open_create_manager;
          }
        "
      >
      </create_manager>
      ...
      <div class="bg-white p-6 flex gap-4 flex-col">
        ...
        <div class="flex flex-col">
          ...
          <div class="flex flex-row gap-2 items-center relative">
            <p class="border border-slate-300 absolute bg-slate-50 h-full flex items-center justify-center px-2">₹</p>
            <input type="text" id="rate-amount" class="p-2 border pl-8" v-model="booking_rate_form.rate_amount" />
          </div>
        </div>
